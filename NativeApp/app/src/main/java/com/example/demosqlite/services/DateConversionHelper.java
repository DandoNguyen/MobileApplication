...
public class DateConversionHelper {
    ...
    public Date useMongoDate(String dateTimeString) {
        try {
            assert dateTimeString != null;
            return sqliteDateTimeFormat.parse(dateTimeString);
        } catch (ParseException e) {
            e.printStackTrace();
            return null;
        }
...
//    }
    ...
    /**
     * create "yyyy-MM-dd HH:mm:ss" Date value from default Date value
     * @param dateTime default Date (any Date)
     * @return "yyyy-MM-dd HH:mm:ss" Date value formatted String
     */
    public String CreateSQLiteDateFormat(Date dateTime) {
        try {
            assert dateTime != null;
            return sqliteDateTimeFormat.format(dateTime);
        } catch (Exception ex) {
            ex.printStackTrace();
            return "";
        }
