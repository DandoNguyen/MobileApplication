...
public class ListTripItemAdapter extends ArrayAdapter<TripModel>{
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (tripModel.getTripImage() != null) {
            Bitmap bitmap = BitmapFactory.decodeByteArray(tripModel.getTripImage(), 0, tripModel.getTripImage().length);
            cv_tripImage.setImageBitmap(bitmap);
        }
        tv_tripName.setText(tripModel.getTripName());
        tv_tripDestination.setText(tripModel.getTripDest());
        tv_tripStartDate.setText(customDate(tripModel.getTripStartDate()));
